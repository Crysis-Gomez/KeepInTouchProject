package events.touch
{
	...
	{
		private var checkDone:Boolean;
		
		public function TouchEvent(playScreen:PlayScreen)
		{
			super(playScreen);
		...
		{
			super.announce();
			var announce:TouchEventAnnouncement = new TouchEventAnnouncement(eventType, playScreen, play);
			announce.start();
		}
		...
		override public function update(playerArea:PlayerArea):void
		{
			...
			if (!playerArea.eventDone)
			{
				if (timeCounter >= timeFinished)  {
					...
					if (!playerArea.previousHasTouchPoint && playerArea.hasTouchPoint)
					{
						playerArea.eventDone = true;
			...
			{
				if (checkIfEveryOneFinished() && !checkDone)
				{
					resolveEvent();
					checkDone = true;
				}
